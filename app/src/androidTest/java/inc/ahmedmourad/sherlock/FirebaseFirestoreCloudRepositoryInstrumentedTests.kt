...
class FirebaseFirestoreCloudRepositoryInstrumentedTests {
    ...
    fun setupRepository() {
        ...
        repository = FirebaseFirestoreCloudRepository(
    ...
    private fun publishChild(child: FirebaseUrlChild): Single<FirebaseUrlChild> {
        return Single.create {
            db.collection(FirebaseContract.Database.PATH_CHILDREN)
                    .document(child.id)
                    .set(child.toMap())
            ...
            db.collection(FirebaseContract.Database.PATH_CHILDREN)
                    ...
                    .addOnCompleteListener { task ->
                        ...
                        if (task.isSuccessful) {
                            ...
                            val result = task.result?.extractFirebaseUrlChild()
                            ...
                            it.onSuccess(DataModelsMapper.toDomainUrlChild(result))
                        } else {
                            it.onError(task.exception
                                    ?: IllegalArgumentException("Exception is null!"))
                        }
